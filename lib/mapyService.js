...
const PROVINCE_BOUNDS = { minLat: 52.85, maxLat: 54.65, minLng: 13.8, maxLng: 17.1 }
  ...
  try {
    ...
    return (data.items || [])
      .filter(item => {
        ...
        return lat >= PROVINCE_BOUNDS.minLat && lat <= PROVINCE_BOUNDS.maxLat &&
               lon >= PROVINCE_BOUNDS.minLng && lon <= PROVINCE_BOUNDS.maxLng
      })
      .map(item => {
