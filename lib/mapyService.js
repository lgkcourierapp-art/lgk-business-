...
export const getRouteSnapshotUrl = ({
  ...
  return (
    ...
    `&shapes=path;color:D4FF00;width:4;${path}` +
    `&markers=color:yellow;size:large;label:A;${fromLng},${fromLat}` +
    `&markers=color:red;size:large;label:B;${toLng},${toLat}`
  )
}
...
export const getBuildingSnapshotUrl = ({
  ...
  return (
    ...
    `&zoom=${zoom}` +
    `&lon=${lng}&lat=${lat}` +
    `&markers=color:yellow;size:large;label:X;${lng},${lat}`
  )
}
