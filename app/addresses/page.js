...
export default function AddressesPage() {
  ...
  const [editing, setEditing] = useState(null) // address object or BLANK
  ...
  const setDefault = async (addr) => {
    // Clear existing defaults of same type, then set new one
    await supabase
      .from('saved_addresses')
    ...
    <div key={lang} style={{ minHeight: '100vh', background: '#0A0A0A' }}>
      ...
      <main style={{ maxWidth: 700, margin: '0 auto', padding: '24px 16px', paddingBottom: 80 }}>
        ...
        {loading ? (
          ...
          <div style={{ ...cardStyle, textAlign: 'center', padding: 48, color: '#555' }}>
            <div style={{ fontSize: 32, marginBottom: 12 }}>📍</div>
            <div style={{ fontWeight: 600, marginBottom: 8 }}>No saved addresses yet</div>
            <div style={{ fontSize: 13, color: '#444' }}>Save your pickup and delivery locations to speed up order placement.</div>
          </div>
        ) : (
          <>
            {[
              ...
              <div key={title} style={{ marginBottom: 28 }}>
                ...
                {list.map(addr => (
                  <div key={addr['id']} style={{ ...cardStyle, borderLeft: addr.is_default_pickup ? '3px solid #D4FF00' : '1px solid #333' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                      <div>
                        ...
                        {addr.contact_name && <div style={{ color: '#666', fontSize: 12, marginTop: 2 }}>{addr.contact_name} · {addr.contact_phone}</div>}
                      </div>
                      <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                        {!addr.is_default_pickup && (
                          <button onClick={() => setDefault(addr)} style={{ background: 'transparent', border: '1px solid #333', color: '#888', padding: '6px 10px', borderRadius: 6, cursor: 'pointer', fontSize: 12 }}>
                            Set default
                          </button>
                        )}
                        <button onClick={() => setEditing({ ...addr })} style={{ background: 'transparent', border: '1px solid #333', color: '#D4FF00', padding: '6px 10px', borderRadius: 6, cursor: 'pointer', fontSize: 12 }}>
                          Edit
                        </button>
                    ...
                    {deleteConfirm === addr['id'] && (
                      <div style={{ marginTop: 12, padding: 12, background: '#FF3B3010', borderRadius: 8, border: '1px solid #FF3B3030', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          ...
                          <button onClick={() => setDeleteConfirm(null)} style={{ background: 'transparent', border: '1px solid #333', color: '#888', padding: '6px 14px', borderRadius: 6, cursor: 'pointer', fontSize: 12 }}>Cancel</button>
                        </div>
                      </div>
                    )}
                  </div>
      ...
      {editing && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 500, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: '#1A1A1A', borderRadius: '16px 16px 0 0', padding: 24, width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div style={{ fontWeight: 700, fontSize: 17, color: '#FFF' }}>
                {editing['id'] ? 'Edit address' : 'Add address'}
              </div>
              <button onClick={() => setEditing(null)} style={{ background: 'transparent', border: 'none', color: '#888', fontSize: 22, cursor: 'pointer' }}>×</button>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
              {['pickup', 'delivery'].map(type => (
                <button
                  ...
                  style={{ flex: 1, padding: '10px 0', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer', border: '1px solid ' + (editing.address_type === type ? '#D4FF00' : '#333'), background: editing.address_type === type ? '#D4FF0015' : 'transparent', color: editing.address_type === type ? '#D4FF00' : '#888' }}
                >
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20, cursor: 'pointer' }}>
              ...
              <span style={{ color: '#CCC', fontSize: 14 }}>Set as default {editing.address_type} address</span>
            </label>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                ...
                style={{ flex: 1, background: (!saving && editing.label && editing.street && editing.city) ? '#D4FF00' : '#333', color: (!saving && editing.label && editing.street && editing.city) ? '#000' : '#666', border: 'none', padding: '14px 0', borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: 'pointer' }}
              >
                {saving ? 'Saving...' : 'Save address'}
              </button>
              <button onClick={() => setEditing(null)} style={{ background: 'transparent', border: '1px solid #333', color: '#888', padding: '14px 20px', borderRadius: 10, cursor: 'pointer', fontSize: 14 }}>
                Cancel
              </button>
            </div>
          </div>
